...
(define (domain rcll-production-durative)
	...
	(:action bs-dispense
		...
		:effect (and
			(wp-at ?wp ?m ?side)
			(not (bs-prepared-color ?m ?basecol))
			(not (bs-prepared-side ?m ?side))
			(not (wp-base-color ?wp BASE_NONE))
			(wp-base-color ?wp ?basecol)
			(not (wp-unused ?wp))
			(wp-usable ?wp)
			(not (mps-state ?m PROCESSING))
	...
	(:action cs-retrieve-cap
		...
		:effect (and
			...
			(cs-buffered ?m ?capcol)
			(cs-can-perform ?m CS_MOUNT)
		)
	)
	
	(:action prepare-rs
		:parameters (?r - robot ?m - mps ?rc - product-ring-color ?rs-before ?rs-after ?r-req - ring-num)
		:precondition (and (at ?r ?m INPUT) (mps-type ?m RS) (mps-state ?m IDLE) (rs-ring-spec ?m ?rc ?r-req) (rs-filled-with ?m ?rs-before) (rs-sub ?rs-before ?r-req ?rs-after))
		:effect (and (not (mps-state ?m IDLE)) (mps-state ?m PREPARED) (rs-prepared-color ?m ?rc))
	)
	...
	(:action rs-mount-ring2
		:parameters (?m - mps ?wp - workpiece ?col - product-ring-color ?col1 - product-ring-color ?rs-before ?rs-after ?r-req - ring-num)
		:precondition (and 
			(mps-type ?m RS)
			(mps-state ?m PROCESSING)
			(wp-at ?wp ?m INPUT)
	...
	(:action rs-mount-ring3
		:parameters (?m - mps ?wp - workpiece ?col - product-ring-color	?col1 ?col2 - product-ring-color ?rs-before ?rs-after ?r-req - ring-num)			 
		:precondition (and
			(mps-type ?m RS)
			(mps-state ?m PROCESSING)
			(wp-at ?wp ?m INPUT)
	...
	(:durative-action move-wp-get
		...
		:duration (= ?duration (path-length ?from ?from-side ?to OUTPUT))
		:condition (and (at start (entered-field ?r))
			(at start (at ?r ?from ?from-side))
			(at start (location-free ?to OUTPUT))
			(at start (mps-state ?to READY-AT-OUTPUT))
			(at start (can-hold ?r)))
		:effect (and (at start (not (at ?r ?from ?from-side)))
			(at start (location-free ?from ?from-side))
			(at start (not (location-free ?to OUTPUT)))
			(at end (at ?r ?to OUTPUT)))
	)
...
;	)
	...
	(:durative-action wp-get-shelf
		...
		:condition (and
			...
			(at start (wp-on-shelf ?cc ?m ?spot))
			(at start (can-hold ?r))
		)
		:effect (and
			(at start (not (at ?r ?m INPUT)))
			(at start (not (can-hold ?r)))
			(at start (not (wp-on-shelf ?cc ?m ?spot))) 
			(at end (wp-usable ?cc))
			(at end (at ?r ?m INPUT))
			(at end (holding ?r ?cc))
		)
	...
	(:durative-action wp-get
		...
		:duration (= ?duration 10)
		:condition (and
			(at start (at ?r ?m ?side))
			(at start (can-hold ?r))
			(at start (wp-at ?wp ?m ?side))
			(at start (mps-state ?m READY-AT-OUTPUT))
			(at start (wp-usable ?wp))
		)
		:effect (and
			(at start (not (at ?r ?m ?side)))
			(at start (not (can-hold ?r)))
			(at start (not (mps-state ?m READY-AT-OUTPUT)))
			(at end (not (wp-at ?wp ?m ?side)))
			(at end (holding ?r ?wp))
			(at end (mps-state ?m IDLE))
			(at end (at ?r ?m ?side))
		)
	)
	...
	(:durative-action wp-put
		...
		:condition (and
			(at start (at ?r ?m INPUT))
			(at start (mps-state ?m PREPARED))
			(at start (wp-usable ?wp))
			(at start (holding ?r ?wp))
		)
		:effect (and
			(at start (not (holding ?r ?wp)))
			(at start (not (at ?r ?m INPUT)))
			(at start (not (mps-state ?m PREPARED)))
	...
	(:durative-action wp-put-slide-cc
		...
		:effect (and
			(at start (not (at ?r ?m INPUT)))
			(at end (not (wp-usable ?wp)))
			(at start (not (holding ?r ?wp)))
			(at end (can-hold ?r))
			(at end (not (rs-filled-with ?m ?rs-before)))
			(at end (rs-filled-with ?m ?rs-after))
			(at end (at ?r ?m INPUT))
		)
	)
